...
def pad_sents_char(sents, char_pad_token):
    ...
    sent_word_padded = max_word_length * [char_pad_token]

    for sent in sents:
        word_padded = []
        for word in sent:
            diff_w = max_word_length - len(word)
            if diff_w >= 0:
                word_padded.append(word + diff_w*[char_pad_token])
            else:
                word_padded.append(word[:max_word_length])

        diff_s = max_sent_len - len(word_padded)
        sents_padded.append(word_padded + [sent_word_padded]*diff_s)
